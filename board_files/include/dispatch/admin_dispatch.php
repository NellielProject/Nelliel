...
function nel_admin_dispatch($inputs)
{
    ...
    switch ($inputs['module'])
    {
        ...
        case 'threads':
            if (isset($_GET['area']) && $_GET['area'] === 'general')
            {
                    ...
                    if ($fgsfds->getCommand('noko') !== false)
                    {
                        if ($sessions->sessionIsActive())
                        {
                            echo '<meta http-equiv="refresh" content="1;URL=' . PHP_SELF .
                                    '?module=modmode&area=view-thread&section=' .
                                    $fgsfds->getCommandData('noko', 'topic') . '&board_id=' . $inputs['board_id'] . '">';
                        }
                        else
                        {
                ...
                else
                {
                    ...
                    {
                        $reports_panel = new \Nelliel\Admin\AdminReports(nel_database(), nel_authorize(),
                                $inputs['board_id']);
                        $reports_panel->actionDispatch($inputs);
            ...
            else
            {
                ...
                $threads_panel = new \Nelliel\Admin\PanelThreads(nel_database(), nel_authorize(), $inputs['board_id']);
                ...
                case 'threads':
                    ...
                    else if ($inputs['action'] === 'sticky' || $inputs['action'] === 'unsticky')
                    {
                        if ($content_id->isPost())
                        {
                            ...
                            $new_thread = new \Nelliel\Content\ContentThread(nel_database(), $new_content_id,
                                    $inputs['board_id']);
                            $new_thread->sticky();
                        }
                        else
