...
function nel_generate_main_panel()
{
    ...
    if ($authorize->get_user_perm($_SESSION['username'], 'perm_config_access', INPUT_BOARD_ID) ||
    ...
    if ($authorize->get_user_perm($_SESSION['username'], 'perm_modmode_access', INPUT_BOARD_ID) ||
         $authorize->get_user_perm($_SESSION['username'], 'perm_all_modmode_access'))
    {
        $dom->removeChild($dom->getElementById('select-mod-mode'));
    }

    if ($authorize->get_user_perm($_SESSION['username'], 'perm_regen_index', INPUT_BOARD_ID) ||
         $authorize->get_user_perm($_SESSION['username'], 'perm_all_regen_index'))
    {
        $dom->removeChild($dom->getElementById('regen-index-form'));
    }

    if ($authorize->get_user_perm($_SESSION['username'], 'perm_regen_caches', INPUT_BOARD_ID) ||
