...
class TableInsertData
{
    ...
    public function permissionsDefaults()
    {
        ...
        $prepared = $dbh->prepare($insert_query);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'perm_site_config_access', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'perm_site_config_modify', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'perm_board_defaults_access', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'perm_board_defaults_modify', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'perm_board_config_access', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'perm_board_config_modify', 1]);
        ...
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'perm_threads_access', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'perm_threads_modify', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'perm_post_delete', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'perm_post_as_staff', 1]);
        ...
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'perm_post_mod_comment', 1]);
        ...
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'perm_file_filters_modify', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'perm_file_filters_delete', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'perm_site_config_access', 0]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'perm_site_config_modify', 0]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'perm_board_defaults_access', 0]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'perm_board_defaults_modify', 0]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'perm_board_config_access', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'perm_board_config_modify', 1]);
        ...
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'perm_threads_access', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'perm_threads_modify', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'perm_post_delete', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'perm_post_as_staff', 1]);
        ...
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'perm_post_mod_comment', 1]);
        ...
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'perm_file_filters_modify', 0]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'perm_file_filters_delete', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'perm_site_config_access', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'perm_site_config_modify', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'perm_board_defaults_access', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'perm_board_defaults_modify', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'perm_board_config_access', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'perm_board_config_modify', 0]);
        ...
        $dbh->executePrepared($prepared, ['MOD', 'perm_threads_access', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'perm_threads_modify', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'perm_post_delete', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'perm_post_as_staff', 1]);
        ...
        $dbh->executePrepared($prepared, ['MOD', 'perm_post_mod_comment', 1]);
        ...
        $dbh->executePrepared($prepared, ['MOD', 'perm_file_filters_modify', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'perm_file_filters_delete', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'perm_site_config_access', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'perm_site_config_modify', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'perm_board_defaults_access', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'perm_board_defaults_modify', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'perm_board_config_access', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'perm_board_config_modify', 0]);
        ...
        $dbh->executePrepared($prepared, ['JANITOR', 'perm_threads_access', 1]);
        $dbh->executePrepared($prepared, ['JANITOR', 'perm_threads_modify', 1]);
        $dbh->executePrepared($prepared, ['JANITOR', 'perm_post_delete', 1]);
        $dbh->executePrepared($prepared, ['JANITOR', 'perm_post_as_staff', 0]);
        ...
        $dbh->executePrepared($prepared, ['JANITOR', 'perm_post_mod_comment', 0]);
        ...
        $dbh->executePrepared($prepared, ['JANITOR', 'perm_file_filters_modify', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'perm_file_filters_delete', 0]);
        nel_setup_stuff_done(true);
    }
