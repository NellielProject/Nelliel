...
class TableInsertData
{
    ...
    public function permissionsDefaults()
    {
        ...
        $prepared = $dbh->prepare($insert_query);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_config_access', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_config_modify', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'site', 'perm_user_access', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'site', 'perm_user_add', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'site', 'perm_user_modify', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'site', 'perm_user_delete', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'site', 'perm_user_change_pass', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'site', 'perm_role_access', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'site', 'perm_role_add', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'site', 'perm_role_modify', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'site', 'perm_role_delete', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_ban_access', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_ban_add', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_ban_modify', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_access', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_modify', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_delete', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_file_delete', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_default_name', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_custom_name', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_override_anon', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_sticky', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_unsticky', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_lock', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_unlock', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_in_locked', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_comment', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_post_permsage', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_regen_caches', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_regen_index', 1]);
        $dbh->executePrepared($prepared, ['SUPER_ADMIN', 'board', 'perm_regen_threads', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_config_access', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_config_modify', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'site', 'perm_user_access', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'site', 'perm_user_add', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'site', 'perm_user_modify', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'site', 'perm_user_delete', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'site', 'perm_user_change_pass', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'site', 'perm_role_access', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'site', 'perm_role_add', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'site', 'perm_role_modify', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'site', 'perm_role_delete', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_ban_access', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_ban_add', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_ban_modify', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_access', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_modify', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_delete', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_file_delete', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_default_name', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_custom_name', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_override_anon', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_sticky', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_unsticky', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_lock', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_unlock', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_in_locked', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_comment', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_post_permsage', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_regen_caches', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_regen_index', 1]);
        $dbh->executePrepared($prepared, ['BOARD_ADMIN', 'board', 'perm_regen_threads', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_config_access', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_config_change', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'site', 'perm_user_access', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'site', 'perm_user_add', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'site', 'perm_user_modify', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'site', 'perm_user_delete', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'site', 'perm_user_change_pass', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'site', 'perm_role_access', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'site', 'perm_role_add', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'site', 'perm_role_modify', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'site', 'perm_role_delete', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_ban_access', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_ban_add', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_ban_modify', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_access', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_modify', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_delete', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_file_delete', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_default_name', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_custom_name', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_override_anon', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_sticky', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_unsticky', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_lock', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_unlock', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_in_locked', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_comment', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_post_permsage', 1]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_regen_caches', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_regen_index', 0]);
        $dbh->executePrepared($prepared, ['MOD', 'board', 'perm_regen_threads', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_config_access', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_config_change', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'site', 'perm_user_access', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'site', 'perm_user_add', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'site', 'perm_user_modify', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'site', 'perm_user_delete', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'site', 'perm_user_change_pass', 1]);
        $dbh->executePrepared($prepared, ['JANITOR', 'site', 'perm_role_access', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'site', 'perm_role_add', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'site', 'perm_role_modify', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'site', 'perm_role_delete', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_ban_access', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_ban_add', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_ban_modify', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_access', 1]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_modify', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_delete', 1]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_file_delete', 1]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_default_name', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_custom_name', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_override_anon', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_sticky', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_unsticky', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_lock', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_unlock', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_in_locked', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_comment', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_post_permsage', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_regen_caches', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_regen_index', 0]);
        $dbh->executePrepared($prepared, ['JANITOR', 'board', 'perm_regen_threads', 0]);
        nel_setup_stuff_done(true);
    }
