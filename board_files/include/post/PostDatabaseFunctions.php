...
class PostDatabaseFunctions
{
    ...
    public function insertInitialPost($time, $post_data)
    {
        $dbh = nel_database();
        ...
        $prepared->bindValue(':secure_tripcode',
                $post_data['secure_tripcode'] === '' ? null : $post_data['secure_tripcode']);
    ...
    public function insertNewThread($thread_info)
    {
        $dbh = nel_database();
        $references = nel_parameters_and_data()->boardReferences($this->board_id);
    ...
    public function updateThread($new_post_info, $thread_info)
    {
        $dbh = nel_database();
        $references = nel_parameters_and_data()->boardReferences($this->board_id);
    ...
    public function insertNewFiles($parent_id, $new_post_info, $files)
    {
        $dbh = nel_database();
        $references = nel_parameters_and_data()->boardReferences($this->board_id);
        $i = 1;
        ...
        foreach ($files as $file)
        {
            $columns = array('parent_thread', 'post_ref', 'file_order', 'type', 'format', 'mime', 'filename',
                'extension', 'image_width', 'image_height', 'preview_name', 'preview_extension', 'preview_width',
                'preview_height', 'filesize', 'md5', 'sha1', 'sha256', 'sha512', 'source', 'license', 'alt_text');
            $values = $dbh->generateParameterIds($columns);
