...
function nel_process_new_post($board_id)
{
    ...
    if(!$spoon)
    {
        ...
        {
            nel_derp(10, nel_stext('Post contains no content or file. Dumbass.'), $board_id);
        }

        if (nel_board_settings($board_id, 'require_image_always'))
        {
            nel_derp(11, nel_stext('Image or file required when making a new post.'), $board_id);
        }

        if (nel_board_settings($board_id, 'require_image_start') && $post_data['response_to'] === 0)
        {
            nel_derp(12, nel_stext('Image or file required to make new thread.'), $board_id);
        }
    }
    ...
    {
        nel_derp(13, nel_stext('Post is too long. Try looking up the word concise.'), $board_id);
    }
...
function nel_is_post_ok($board_id, $post_data, $time)
{
    ...
    if ($renzoku > 0)
    {
        nel_derp(1, nel_stext('Flood detected! You\'re posting too fast, slow the fuck down.'), $board_id);
    }
    ...
    if ($post_data['parent_thread'] != 0)
    {
        ...
        if (!empty($op_post))
        {
            if ($op_post['locked'] == 1)
            {
                nel_derp(2, nel_stext('This thread is locked.'), $board_id);
            }

            if ($op_post['archive_status'] != 0)
            {
                nel_derp(3, nel_stext('The thread you have tried posting in is currently inaccessible or archived.'), $board_id);
            }
        ...
        {
            nel_derp(4, nel_stext('The thread you have tried posting in could not be found.'), $board_id);
        }

        if ($post_count >= nel_board_settings($board_id, 'max_posts'))
        {
            nel_derp(5, nel_stext('The thread has reached maximum posts.'), $board_id);
        }

        if ($op_post['archive_status'] != 0)
        {
            nel_derp(6, nel_stext('The thread is archived or buffered and cannot be posted to.'), $board_id);
        }
    }
