...
class AuthPermissions extends AuthBase
{
    ...
    public function loadFromDatabase($temp_database = null)
    {
        ...
        $prepared = $database->prepare('SELECT * FROM "' . PERMISSIONS_TABLE . '" WHERE "role_id" = ?');
    ...
    public function writeToDatabase($temp_database = null)
    {
        ...
        {
            $prepared = $database->prepare('SELECT "entry" FROM "' . PERMISSIONS_TABLE . '" WHERE "role_id" = ? AND "perm_id" = ? LIMIT 1');
