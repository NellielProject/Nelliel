...
{{ else }}
    ...
    {{ if !$rendervar['response_id'] }}
        {{ if $rendervar['logged_in'] }}
            [<a href="{PHP_SELF}?mode=display&post={$rendervar['post_number']}">{$lang['LINK_REPLY']}</a>]
        {{ else }}
            [<a href="{PAGE_DIR}{$rendervar['post_number']}/{$rendervar['post_number']}.html">{$lang['LINK_REPLY']}</a>]
        {{ endif }}
    {{ endif }}
    {{ if $rendervar['expand_post'] && $rendervar['logged_in'] }}
            [<a href="javascript:clientSideInclude('expand{$rendervar['post_number']}', 'expLink{$rendervar['post_number']}', '{PHP_SELF}?mode=display&post={$rendervar['post_number']}&expand=TRUE', '{PHP_SELF}?mode=display&post={$rendervar['post_number']}&collapse=TRUE', 'Collapse thread')" id="expLink{$rendervar['post_number']}">{$lang['THREAD_EXPAND']}</a>]
    {{ elseif $rendervar['expand_post'] && !$rendervar['logged_in'] }}
            [<a href="javascript:clientSideInclude('expand{$rendervar['post_number']}', 'expLink{$rendervar['post_number']}', '{PAGE_DIR}{$rendervar['post_number']}/{$rendervar['post_number']}-expand.html', '{PAGE_DIR}{$rendervar['post_number']}/{$rendervar['post_number']}-collapse.html', 'Collapse thread')" id="expLink{$rendervar['post_number']}">{$lang['THREAD_EXPAND']}</a>]
    {{ endif }}
