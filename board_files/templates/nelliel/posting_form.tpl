...
<div>
    <div class="posting-form">
        <form accept-charset="utf-8" name="postingform" action="{$rendervar['form_submit_url']}" method="post" enctype="multipart/form-data">
            <div>
                <input type="hidden" name="mode" value="new_post"> {{ if $rendervar['modmode'] }} <input type="hidden" name="mode2"
                    value="modmode"> {{ endif }} <input type="hidden" name="response_to" value="{$rendervar['response_id']}">
            </div>
            <table class="input-table">
                ...
                <tr class="posting-form-row">
                    ...
                    <td class="posting-form-input"><input type="text" name="notanonymous" id="durrname" size="40"
                        maxlength="{BS_MAX_NAME_LENGTH}"></td>
                </tr>
                <tr class="posting-form-row">
                    <td class="posting-form-label"><label for="durrmail">{LANG_LABEL_EMAIL}</label></td>
                    <td class="posting-form-input"><input type="text" name="spamtarget" id="durrmail" size="40"
                        maxlength="{BS_MAX_EMAIL_LENGTH}"></td>
                </tr>
                {{ endif }}
                <tr class="posting-form-row">
                    <td class="posting-form-label"><label for="durrsubject">{LANG_LABEL_SUBJECT}</label></td>
                    <td class="posting-form-input"><input type="text" name="verb" id="durrsubject" size="40"
                        maxlength="{BS_MAX_SUBJECT_LENGTH}"></td>
                </tr>
                <tr class="posting-form-row">
                ...
                {{ for $i = 1, $j = 2; $i <= BS_MAX_POST_FILES; ++$i, ++$j }} {{ if $i === 1 }}
                <tr class="posting-form-row" id="file{$i}">{{ else }}
                <tr class="posting-form-row none" id="file{$i}">
                    {{ endif }}
                    <td class="posting-form-label"><label for="durrfile{$i}">{LANG_LABEL_FILE} #{$i}</label></td>
                    <td class="posting-form-input"><input type="file" name="upfile{$i}" id="durrfile{$i}"
                        onchange="addMoarInput('file{$j}',false)">&nbsp; <input type="button" value="Add Source" id="addsrc{$i}"
                        onClick="addMoarInput('src{$i}',true)">&nbsp; <input type="button" value="Add License" id="addlcns{$i}"
                        onClick="addMoarInput('lcns{$i}',true)"></td>
                </tr>
                ...
                <tr class="posting-form-row">
                    ...
                    <td class="posting-form-input"><input type="submit" value="{LANG_FORM_SUBMIT}">&nbsp;&nbsp;&nbsp;&nbsp;<input
                        type="reset" value="{LANG_FORM_RESET}"></td>
                </tr>
                <tr class="posting-form-row">
                    {{ if $rendervar['response_id'] > 0 }}
                    <td class="posting-form-label"></td>
                    <td class="posting-form-input">{LANG_TEXT_REPLYMODE}</td> {{ else }}
                    <td class="posting-form-label"></td>
                    <td class="posting-form-input">{LANG_TEXT_THREADMODE}</td> {{ endif }}
                </tr>
                <tr class="posting-form-row">
                ...
                <tr class="none">
                    <td><label for="thename1">{LANG_TEXT_SPAMBOT_TRAP}</label></td>
                    <td><input type="text" name="{LANG_TEXT_SPAMBOT_FIELD1}" id="thename1" size="30"></td>
                </tr>
                <tr class="none">
                    <td><label for="theurl1">{LANG_TEXT_SPAMBOT_TRAP}</label></td>
                    <td><input type="text" name="{LANG_TEXT_SPAMBOT_FIELD2}" id="theurl1" size="60"></td>
                </tr>
                {{ endif }}
            </table>
        </form>
