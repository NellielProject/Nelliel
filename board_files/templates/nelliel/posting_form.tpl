{{ if nel_render_out('response_id') }}
        <div>
            [<a href="{nel_render_out('dotdot')}{nel_render_out('page_ref1')}">{nel_stext('LINK_RETURN')}</a>]
        </div>
{{ endif }}
    <div>
        <div class="posting-form">
            <form accept-charset="utf-8" name="postingform" action="{nel_render_out('form_submit_url')}" method="post" enctype="multipart/form-data">
                <div>
                    <input type="hidden" name="mode" value="new_post">
{{ if nel_render_out('modmode') }}
                    <input type="hidden" name="mode2" value="modmode">
{{ endif }}
                    <input type="hidden" name="response_to" value="{nel_render_out('response_id')}">
                </div>    
...
{{ endif }}
                    ...
                    <tr class="posting-form-row">
{{ if nel_render_out('response_id') > 0 }}
                        <td class="posting-form-label"></td>
                        <td class="posting-form-input">
