...
{{ if $rendervar['thread_panel_loop'] }}
    ...
    {{ if $rendervar['sticky'] }}
        <td></td>
        <td><input type="checkbox" name="{$rendervar['post_number']}" value="unsticky_{$rendervar['post_number']}"></td>
    {{ else }}
    ...
    {{ endif }}
        ...
        <td>{$rendervar['name']}</td><td>{$rendervar['comment']}</td><td>{$rendervar['host']}</td>
    ...
    {{ foreach $rendervar['files'] as $file }}
        ...
        {{ else }}
        <td><a href="{SRC_DIR}{$rendervar['response_to']}/{$file['filename']}{$file['extension']}" rel="external">{$file['filename']}{$file['extension']}</a> ( {$file['filesize']} KB )<br>MD5: {$file['md5']}</td></tr>
        {{ endif }}
    {{ endforeach }}
        </table>
        </td>
