{{ if $rendervar['thread_panel_form'] }}
    ...
    {{ if $rendervar['thread_panel_loop'] }}
        ...
        {{ endif }}
            	...
            	<td>
        {{ if $rendervar['is_op'] }}
                	<input type="checkbox" name="thread_{$rendervar['post_number']}" value="deletethread_{$rendervar['post_number']}" title="Delete entire post">(OP)
        {{ else }}
                	<input type="checkbox" name="post_{$rendervar['post_number']}_{$rendervar['response_to']}" value="deletepost_{$rendervar['post_number']}_{$rendervar['response_to']}" title="Delete entire post">
        {{ endif }}
            	</td>
        {{ if $rendervar['sticky'] }}
            	<td>
            	...
            	<td>
            		<input type="checkbox" name="{$rendervar['post_number']}" value="unsticky_{$rendervar['post_number']}">
            	</td>
        {{ else }}
            	<td>
            		<input type="checkbox" name="{$rendervar['post_number']}" value="sticky_{$rendervar['post_number']}">
            	</td>
            	<td>
            	</td>
        {{ endif }}
            	...
            	<td>
            		<a href="{PAGE_DIR}{$rendervar['post_number']}/{$rendervar['post_number']}.html" rel="external">{$rendervar['subject']}</a>
            	</td>
            	<td>
            		{$rendervar['name']}
            	</td>
        ...
        {{ if $rendervar['has_file'] }}
        	...
        	{{ foreach $rendervar['files'] as $file }}
            			<tr>
            				<td>
            					<input type="checkbox" name="fileid{$rendervar['post_number']}_{$file['file_order']}" value="deletefile_{$rendervar['post_number']}_{$file['file_order']}" title="Delete file">
            				</td>
            	{{ if $rendervar['response_to'] == 0 }}
            				<td>
            					<a href="{SRC_DIR}{$rendervar['post_number']}/{$file['filename']}{$file['extension']}" rel="external">{$file['filename']}{$file['extension']}</a> ( {$file['filesize']} KB )<br>MD5: {$file['md5']}
            				</td>
            			</tr>
            	{{ else }}
            				<td>
            					<a href="{SRC_DIR}{$rendervar['response_to']}/{$file['filename']}{$file['extension']}" rel="external">{$file['filename']}{$file['extension']}</a> ( {$file['filesize']} KB )<br>MD5: {$file['md5']}
            				</td>
            			</tr>
            	{{ endif }}
        	{{ endforeach }}
