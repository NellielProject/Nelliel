...
class ROBOT9000
{
    ...
    public function checkForHash(DomainBoard $board, string $hash): bool
    {
        if ($board->setting('r9k_global_unoriginal_check')) {
            $prepared = nel_get_cached_domain(Domain::SITE)->database()->prepare(
        ...
        $result = nel_get_cached_domain(Domain::SITE)->database()->executePreparedFetch($prepared, null, PDO::FETCH_COLUMN);
        return $result !== false;
    }
    ...
    public function addHash(DomainBoard $board, string $hash, int $time): void
    {
        ...
        nel_get_cached_domain(Domain::SITE)->database()->executePrepared($prepared, null);
    }
