...
class TableFiletypes extends Table
{
    ...
    public function insertDefaults()
    {
        ...
        $this->insertDefaultRow(['bmp', '["bmp"]', 'graphics', 'image/bmp', '^BM.{10}\x00\x00.{1}\x00\x00\x00.{8}\x01\x00.{1}\x00', 'BMP', 1]);
        $this->insertDefaultRow(['icon', '["ico"]', 'graphics', 'image/x-icon', '^\x00\x00\x01\x00', 'Icon', 1]);
        $this->insertDefaultRow(['photoshop', '["psd"]', 'graphics', 'image/vnd.adobe.photoshop', '^8BPS\x00\x01', 'Photoshop', 1]);
        $this->insertDefaultRow(['tga', '["tga"]', 'graphics', 'image/x-tga', '^.{1}\x01[\x01\x09].{4}[\x0F\x10\x18\x20]|^.{1}\x00[\x02\x03\x0A\x0B]\x00{5}|TRUEVISION-XFILE.\x00$', 'Truevision TGA', 1]);
        $this->insertDefaultRow(['pict', '["pct", "pic"]', 'graphics', 'image/x-pict', '^.{522}(?:\x11\x01|\x00\x11\x02\xFF\x0C\x00\xFF)', 'QuickDraw/PICT', 1]);
        ...
        // Audio formats
        $this->insertDefaultRow(['wave', '["wav"]', 'audio', 'audio/x-wav', '^RIFF.{4}WAVEfmt', 'WAVE', 1]);
        $this->insertDefaultRow(['aiff', '["aif","aiff"]', 'audio', 'audio/x-aiff', '^FORM.{4}AIFF', 'AIFF', 1]);
        $this->insertDefaultRow(['mp3', '["mp3"]', 'audio', 'audio/mpeg', '^ID3|\xFF[\xE3\xF2\xF3\xFA\FB]{1}', 'MP3', 1]);
        $this->insertDefaultRow(['m4a', '["m4a"]', 'audio', 'audio/x-m4a', '^.{4}ftypM4A', 'MPEG-4 Audio', 1]);
        $this->insertDefaultRow(['flac', '["flac"]', 'audio', 'audio/x-flac', '^fLaC\x00\x00\x00\x22', 'FLAC', 1]);
        ...
        $this->insertDefaultRow(['midi', '["mid","midi"]', 'audio', 'audio/midi', '^MThd\x00{3}\x06\x00[\x00-\x02].{4}MTrk', 'MIDI', 1]);
        ...
        $this->insertDefaultRow(['3gp', '["3gp"]', 'video', 'video/3gpp', '^.{4}ftyp3gp', '3GP', 1]);
        $this->insertDefaultRow(['ogg-video', '["ogv"]', 'video', 'video/ogg', '^OggS', 'Ogg Video', 1]);

        // General document formats
        $this->insertDefaultRow(['rtf', '["rtf"]', 'document', 'text/rtf', '{\\rtf[1]?', 'Rich Text', 1]);
        $this->insertDefaultRow(['pdf', '["pdf"]', 'document', 'application/pdf', '^%PDF-[0-9]\.[0-9]', 'PDF', 1]);
        $this->insertDefaultRow(['doc', '["doc"]', 'document', 'application/msword', '^\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1.{20}\xFE\xFF', 'MS Word', 1]);
        ...
        // Archive formats
        $this->insertDefaultRow(['gzip', '["gz","tgz","gzip"]', 'archive', 'application/x-gzip', '^\x1F[\x8B\x9E]\x08', 'GZip', 1]);
        ...
        $this->insertDefaultRow(['rar', '["rar"]', 'archive', 'application/vnd.rar', '^Rar\x21\x1A\x07[\x00\x01]', 'RAR', 1]);
        $this->insertDefaultRow(['stuffit', '["sit","sitx"]', 'archive', 'application/x-stuffit', '^StuffIt|^SIT\!', 'StuffIt', 1]);
        ...
        // Other formats
        $this->insertDefaultRow(['swf', '["swf"]', 'other', 'application/x-shockwave-flash', '^CWS|FWS|ZWS', 'Flash/Shockwave', 1]);
