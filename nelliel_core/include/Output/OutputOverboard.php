...
class OutputOverboard extends OutputCore
{
    ...
    public function render(array $parameters, bool $data_only)
    {
        ...
        if (empty($thread_list))
        {
            ...
            if ($this->write_mode)
            {
                $this->file_handler->writeFile(
                        NEL_BASE_PATH . $this->site_domain->setting('overboard_uri') . '/' . 'index' . NEL_PAGE_EXT,
                        $output, NEL_FILES_PERM, true);
                $json_index->storeData($json_index->prepareData($gen_data['index']), 'index');
                $json_index->writeStoredData($this->site_domain->setting('overboard_uri'), 'index');
            }
            else
            {
                echo $output;
        ...
        foreach ($thread_list as $thread)
        {
            ...
            $thread_board_data = $this->database->executePreparedFetch($prepared, [$thread['board_id']],
                    PDO::FETCH_ASSOC);
            ...
            $treeline = $this->database->executePreparedFetchAll($prepared,
                    [$thread['thread_id']], PDO::FETCH_ASSOC);
            ...
            if ($threads_on_page >= $this->site_domain->setting('overboard_threads') || $threads_done == $thread_count)
            {
                ...
                {
                    $this->file_handler->writeFile(
                            NEL_BASE_PATH . $this->site_domain->setting('overboard_uri') . '/' . $index_filename,
                            $output, NEL_FILES_PERM, true);
                    $json_index->storeData($json_index->prepareData($gen_data['index']), 'index');
                    $json_index->writeStoredData(NEL_BASE_PATH . $this->site_domain->setting('overboard_uri') . '/',
                            'index');
                }
                else
                {
