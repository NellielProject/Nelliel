...
class OutputPanelRoles extends OutputCore
{
    ...
    private function renderPanel(array $parameters, bool $data_only)
    {
        ...
        foreach ($roles as $role)
        {
            ...
            $role_data['edit_url'] = NEL_MAIN_SCRIPT . '?module=admin&section=roles&actions=edit&role-id=' . $role['role_id'];
            $role_row_nodes['remove_url'] = NEL_MAIN_SCRIPT . '?module=admin&section=roles&actions=remove&role-id=' . $role['role_id'];
            $this->render_data['roles_list'][] = $role_data;
        }

        $this->render_data['new_role_url'] = NEL_MAIN_SCRIPT . '?module=admin&section=roles&actions=new';
    ...
    private function renderEdit(array $parameters, bool $data_only)
    {
        ...
        if (empty($role_id))
        {
            $this->render_data['form_action'] = NEL_MAIN_SCRIPT . '?module=admin&section=roles&actions=add';
        }
        else
        {
            $this->render_data['form_action'] = NEL_MAIN_SCRIPT . '?module=admin&section=roles&actions=update&role-id=' . $role_id;
        }
